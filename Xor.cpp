...
Xor::Xor(Common com, queue<short> &instruction, map<string, int> &registers, map<string, int> &memories32bit, map<string, int16_t> &memories16bit, map<string, int8_t> &memories8bit, list<string> &memoryAccesses)
        ...
        else
        {
            if (w == 0)
            {
                if (d == 0)
                {
                    if (index == 4)
                    {
                        memoryAccesses.push_back("xor "  "(%" + regs_32[base] + "),%" + regs_8[reg]);
