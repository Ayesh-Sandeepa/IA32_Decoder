...
And_andOverride::And_andOverride(Common com, queue<short> &instruction, map<string, int> &registers, map<string, int> &memories32bit, map<string, int16_t> &memories16bit, map<string, int8_t> &memories8bit, list<string> &memoryAccesses)
...
string And_andOverride::decode_displacement_without_SIB(int w, int d, int mod, int reg, int rm)
{
...
string And_andOverride::decode_mod_00(int w, int d, int reg, int rm)
{
    string string00 = "";
    if (rm == 6)
    {
...
string And_andOverride::decode_mod_01(int w, int d, int reg, int rm)
{
    string string01 = decode_displacement_without_SIB(w, d, 1, reg, rm);
    return string01;
}

string And_andOverride::decode_mod_10(int w, int d, int reg, int rm)
{
    string string10 = decode_displacement_without_SIB(w, d, 2, reg, rm);
    return string10;
}

string And_andOverride::decode_mod_11(int w, int d, int reg, int rm)
{
    string string11 = "";
    if (w == 0)
    {
...
string And_andOverride::decode_imm(int opCode, int w, int d)
{
    string dec_imm;
    int imm;
...
string And_andOverride::decode_and(short prefixes[4])
{
    ...
    else
    {
        ...
        if (immediate)
        {
            And_override_immediate and_override_immediate(common, instruction, registers, memories32bit, memories16bit, memories8bit, memoryAccesses);
            decoded_bytes =  and_override_immediate.decode_imm(prefixes, w, d, mod, rm);
        }
        else
        {
            if (mod == 0)
