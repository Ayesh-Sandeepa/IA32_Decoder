...
Cmp_override_immediate::Cmp_override_immediate(Common com, queue<short> &instruction, map<string, int> &registers, map<string, int> &memories32bit, map<string, int16_t> &memories16bit, map<string, int8_t> &memories8bit, list<string> &memoryAccesses)
...
string Cmp_override_immediate::decode_displacement_without_SIB(int w, int mod, int rm)
{
    ...
    if (mod == 0)
    {
        if (w == 0)
        {
            ...
            memoryAccesses.push_back("cmp " + common.getHex(disp, 0, 0) + ", $" + common.getHex(imm, 0, 0));
    ...
    else
    {
        ...
        {
            int8_t imm = common.assemble_bits(1, instruction, registers);
            memoryAccesses.push_back("cmp " + common.getHex(disp, 0, 0) + "(%" + regs_16bitmode[rm] + ")" + ", $" + common.getHex(imm, 0, 0));
...
string Cmp_override_immediate::decode_mod_00(int w, int rm)
{
    string string00 = "";
    if (rm == 6)
    {
    ...
    else
    {
        ...
        {
            int8_t imm = common.assemble_bits(1, instruction, registers);
            memoryAccesses.push_back("cmp (%" + regs_16bitmode[rm] + ")"+", $" + common.getHex(imm, 0, 0));
...
string Cmp_override_immediate::decode_mod_01(int w, int rm)
{
    string string01 = decode_displacement_without_SIB(w, 1, rm);
    return string01;
}

string Cmp_override_immediate::decode_mod_10(int w, int rm)
{
    string string10 = decode_displacement_without_SIB(w, 2, rm);
    return string10;
}

string Cmp_override_immediate::decode_mod_11(int w, int rm)
{
    string string11 = "";
    if (w == 0)
    {
...
string Cmp_override_immediate::decode_imm(short prefixes[4], int w, int mod, int rm)
{
