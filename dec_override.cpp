...
Dec_override::Dec_override(Common com, queue<short> &instruction, map<string, int> &registers, map<string, int> &memories32bit, map<string, int16_t> &memories16bit, map<string, int8_t> &memories8bit, list<string> &memoryAccesses)
...
string Dec_override::decode_displacement_without_SIB(int w, int mod, int rm)
{
    ...
    if (mod == 0)
    {
        if (w == 0)
        {
            ...
            memoryAccesses.push_back("dec " + common.getHex(disp, 0, 0));
    ...
    else
    {
        if (w == 0)
        {
            memoryAccesses.push_back("dec " +common.getHex(disp, 0, 0) + "(%" + regs_16bitmode[rm] + ")");
            int8_t num1, num2;
...
string Dec_override::decode_mod_00(int w, int rm)
{
    string string00 = "";
    if (rm == 6)
    {
    ...
    else
    {
        if (w == 0)
        {
            memoryAccesses.push_back("dec (%" + regs_16bitmode[rm] + "),%" );
...
string Dec_override::decode_mod_01(int w, int rm)
{
    string string01 = decode_displacement_without_SIB(w, 1, rm);
    return string01;
}

string Dec_override::decode_mod_10(int w, int rm)
{
    string string10 = decode_displacement_without_SIB(w, 2, rm);
    return string10;
}

string Dec_override::decode_mod_11(int w, int rm)
{
    string string11 = "";
    int8_t num1, num2;
    if (w == 0)
...
string Dec_override::decode_dec(short prefixes[4], int w)
{
    printf ("Dec when address override is present\n");
