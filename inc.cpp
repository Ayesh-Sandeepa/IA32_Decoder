...
string Inc::decode_displacement_with_SIB(int w, int mod, int index, int scale, int base)
{
    string dispWithSIB = "";
    ...
    if (mod == 0)
    {
        ...
        else
        {
            if (w == 0)
            {
                ...
                {
                    memoryAccesses.push_back("inc (%" + regs_32[base] + ",%" + regs_32[index] + "," + to_string(scale) + ")");
...
string Inc::decode_displacement_without_SIB(int w, int mod, int rm)
{
...
string Inc::decode_SIB(int w, int mod)
{
    string stringSib = "";
...
string Inc::decode_mod_00(int w, int rm)
{
    string string00 = "";
    if (rm == 4)
    {
    ...
    else
    {
        ...
        {
            int8_t num1, num2;
            memoryAccesses.push_back("inc (%" + regs_32[rm] + ")");
            string00 = "(%" + regs_32[rm] + ")\n";
...
string Inc::decode_mod_01(int w, int rm)
{
    string string01 = "";
    if (rm == 4)
    {
...
string Inc::decode_mod_10(int w, int rm)
{
    string string10 = "";
    if (rm == 4)
    {
...
string Inc::decode_mod_11(int w, int rm)
{
    string string11 = "";
    int8_t num1, num2;
    if (w == 0)
...
string Inc::decode_inc(short prefixes[4])
{
    string decoded_bytes;

    if (prefixes[3] == 0x67)
    {
        Inc_override Inc_override(common, instruction, registers, memories32bit, memories16bit, memories8bit, memoryAccesses);
        decoded_bytes = Inc_override.decode_inc(prefixes);
    ...
    return "Inc/Dec instantiated and done\n";
}
