...
Neg_addOverride::Neg_addOverride(Common com, queue<short> &instruction, map<string, int> &registers, map<string, int> &memories32bit, map<string, int16_t> &memories16bit, map<string, int8_t> &memories8bit, list<string> &memoryAccesses)
...
string Neg_addOverride::decode_displacement_without_SIB(int w, int mod, int rm)
{
    ...
    if (mod == 0)
    {
        if (w == 0)
        {
                memoryAccesses.push_back("neg " + common.getHex((unsigned int)disp, 0, 0) );
    ...
    else
    {
        if (w == 0)
        {
                memoryAccesses.push_back("neg " + common.getHex((unsigned int)disp, 0, 0) + "(%" + regs_16bitmode[rm] + ")" );
...
string Neg_addOverride::decode_mod_00(int w, int rm)
{
    string string00 = "";
    if (rm == 6)
    {
    ...
    else
    {
        ...
        else if (w == 1)
        {
            if (opSize)
            {
                    memoryAccesses.push_back("neg (%" + regs_16bitmode[rm] + ")");
                    string00 = "(%" + regs_16bitmode[rm] + ")" +  "\n";
...
string Neg_addOverride::decode_mod_01(int w, int rm)
{
    string string01 = decode_displacement_without_SIB(w, 1, rm);
    return string01;
}

string Neg_addOverride::decode_mod_10(int w, int rm)
{
    string string10 = decode_displacement_without_SIB(w, 2, rm);
    return string10;
}

string Neg_addOverride::decode_mod_11(int w, int rm)
{
    string string11 = "";
    if (w == 0)
    { // printf("w:0 and d:0 \n");
...
string Neg_addOverride::decode_neg(short prefixes[4])
{
    printf("Neg when address override is present\n");
