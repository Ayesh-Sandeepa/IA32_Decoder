...
Right_shift::Right_shift(Common com, queue<short> &instruction, map<string, int> &registers, map<string, int> &memories32bit, map<string, int16_t> &memories16bit, map<string, int8_t> &memories8bit, list<string> &memoryAccesses, ofstream &myoutput)
...
string Right_shift::decode_displacement_with_SIB(int d, int w, int mod, int index, int scale, int base)
{
    string dispWithSIB = "";
    ...
    if (mod == 0)
    {
        if (base == 5)
        {
            if (w == 0 and d == 0)
            {
                if (index == 4)
                {
                    memoryAccesses.push_back("shr " + st);
                ...
                {
                    memoryAccesses.push_back("shr %" + st + "(,%" + regs_32[index] + "," + to_string(scale) + ")");
        ...
        else
        {
            if (w == 0 and d == 0)
            {
                ...
                {
                    memoryAccesses.push_back("shr (%" + regs_32[base] + ",%" + regs_32[index] + "," + to_string(scale) + ")");
            ...
            else if (w == 1 and d == 0)
            {
                if (opSize)
                {
                    if (index == 4)
                    {
                        memoryAccesses.push_back("shr (%" + regs_32[base] + ")");
    ...
    else
    {
        if (w == 0 and d == 0)
        {
            if (index == 4)
            {
                memoryAccesses.push_back("shr " + st + "(%" + regs_32[base] + ")");
            ...
            {
                memoryAccesses.push_back("shr " + st + "(%" + regs_32[base] + ",%" + regs_32[index] + "," + to_string(scale) + ")");
...
string Right_shift::decode_displacement_without_SIB(int d, int w, int mod, int rm)
{
    ...
    else
    {
        if (w == 0 and d == 0)
        {
            memoryAccesses.push_back("shr " + st + "(%" + regs_32[rm] + ")");
...
string Right_shift::decode_SIB(int d, int w, int mod)
{
    string stringSib = "";
...
string Right_shift::decode_mod_00(int d, int w, int rm)
{
    string string00 = "";
    if (rm == 4)
    {
    ...
    else
    {
        ...
        {
            int8_t num1;
            memoryAccesses.push_back("shr (%" + regs_32[rm] + ")");
            string00 = "(%" + regs_32[rm] + ")\n";
...
string Right_shift::decode_mod_01(int d, int w, int rm)
{
    string string01 = "";
    if (rm == 4)
    {
...
string Right_shift::decode_mod_10(int d, int w, int rm)
{
    string string10 = "";
    if (rm == 4)
    {
...
string Right_shift::decode_mod_11(int d, int w, int rm)
{
    string string11 = "";
    int8_t num1;
    if (w == 0 and d == 0)
...
string Right_shift::decode_shr(short prefixes[4], int d, int w)
{
