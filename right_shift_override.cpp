...
Right_shift_override::Right_shift_override(Common com, queue<short> &instruction, map<string, int> &registers, map<string, int> &memories32bit, map<string, int16_t> &memories16bit, map<string, int8_t> &memories8bit, list<string> &memoryAccesses)
...
string Right_shift_override::decode_displacement_without_SIB(int d, int w, int mod, int rm)
{
    ...
    if (mod == 0)
    {
        if (w == 0 and d == 0)
        {
            ...
            memoryAccesses.push_back("shr " + common.getHex(disp, 0, 0));
    ...
    else
    {
        if (w == 0 and d == 0)
        {
            memoryAccesses.push_back("shr " +common.getHex(disp, 0, 0) + "(%" + regs_16bitmode[rm] + ")");
            int8_t num1;
            int num2;
            if (rm < 4)
            {
...
string Right_shift_override::decode_mod_00(int d, int w, int rm)
{
    string string00 = "";
    if (rm == 6)
    {
    ...
    else
    {
        if (w == 0 and d == 0)
        {
            memoryAccesses.push_back("shr (%" + regs_16bitmode[rm] + "),%" );
...
string Right_shift_override::decode_mod_01(int d, int w, int rm)
{
    string string01 = decode_displacement_without_SIB(d, w, 1, rm);
    return string01;
}

string Right_shift_override::decode_mod_10(int d, int w, int rm)
{
    string string10 = decode_displacement_without_SIB(d, w, 2, rm);
    return string10;
}

string Right_shift_override::decode_mod_11(int d, int w, int rm)
{
    string string11 = "";
    int8_t num1;
    if (w == 0 and d == 0)
...
string Right_shift_override::decode_shr(short prefixes[4], int d, int w)
{
    printf ("shr when address override is present\n");
