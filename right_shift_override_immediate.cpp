...
Right_shift_override_immediate::Right_shift_override_immediate(Common com, queue<short> &instruction, map<string, int> &registers, map<string, int> &memories32bit, map<string, int16_t> &memories16bit, map<string, int8_t> &memories8bit, list<string> &memoryAccesses)
...
string Right_shift_override_immediate::decode_displacement_without_SIB(int w, int mod, int rm)
{
    ...
    if (mod == 0)
    {
        if (w == 0)
        {
            ...
            num2 = num1 >> imm;
            memoryAccesses.push_back("shr " + common.getHex(disp, 0, 0) + ", $" + common.getHex(unmaskedImm, 0, 0));
    ...
    else
    {
        if (w == 0)
        {
            ...
            memoryAccesses.push_back("shr " + common.getHex(disp, 0, 0) + "(%" + regs_16bitmode[rm] + ")" + ", $" + common.getHex(unmaskedImm, 0, 0));
...
string Right_shift_override_immediate::decode_mod_00(int w, int rm)
{
    string string00 = "";
    if (rm == 6)
    {
    ...
    else
    {
        ...
        if (w == 0)
        {
            memoryAccesses.push_back("shr (%" + regs_16bitmode[rm] + ")"+", $" + common.getHex(unmaskedImm, 0, 0));
...
string Right_shift_override_immediate::decode_mod_01(int w, int rm)
{
    string string01 = decode_displacement_without_SIB(w, 1, rm);
    return string01;
}

string Right_shift_override_immediate::decode_mod_10(int w, int rm)
{
    string string10 = decode_displacement_without_SIB(w, 2, rm);
    return string10;
}

string Right_shift_override_immediate::decode_mod_11(int w, int rm)
{
    int8_t unmaskedImm = common.assemble_bits(1, instruction, registers);
    int imm = common.get_bits(1, 5, unmaskedImm);
    string string11 = "";
...
string Right_shift_override_immediate::decode_shr(short prefixes[4], int w,int mod, int rm)
{
